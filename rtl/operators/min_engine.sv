// ===========================================================================
//  min_engine.sv -- streaming MIN aggregation.
//
//  Sink that consumes one pipeline_data_t stream per query and keeps the
//  minimum value of the configured column over the rows whose pass bit is
//  set. The result is latched when the final beat (tlast) is consumed.
//
//  The accumulator is initialized to all-ones (the largest representable
//  value), so it is updated by the first passing row. If no row passes, the
//  result remains all-ones (documented sentinel).
//
...
module min_engine #(
    ...
    input logic start,
    input logic [COL_ADDR_W-1:0] column,  // column to take the minimum of
  ...
  // Next accumulator value (pass-qualified minimum).
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      acc_q    <= {COLUMN_W{1'b1}};   // identity for MIN
      result_q <= '0;
      busy_q   <= 1'b1;
      done_q   <= 1'b0;
...
`ifdef DBQA_ASSERT
  // The running minimum never exceeds the last presented row's value.
  m_min_monotonic :
