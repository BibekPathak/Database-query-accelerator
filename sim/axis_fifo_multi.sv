...
module axis_fifo_multi #(
    ...
    axis_fifo #(.DEPTH(1), .DATA_W(DATA_W)) u_d1 (
        .clk(clk), .rst(rst),
        .s_axis_tvalid(s0_axis_tvalid), .s_axis_tready(s0_axis_tready),
        .s_axis_tdata (s0_axis_tdata),  .s_axis_tlast (s0_axis_tlast),
        .m_axis_tvalid(m0_axis_tvalid), .m_axis_tready(m0_axis_tready),
        .m_axis_tdata (m0_axis_tdata),  .m_axis_tlast (m0_axis_tlast)
    );

    axis_fifo #(.DEPTH(3), .DATA_W(DATA_W)) u_d3 (
        .clk(clk), .rst(rst),
        .s_axis_tvalid(s1_axis_tvalid), .s_axis_tready(s1_axis_tready),
        .s_axis_tdata (s1_axis_tdata),  .s_axis_tlast (s1_axis_tlast),
        .m_axis_tvalid(m1_axis_tvalid), .m_axis_tready(m1_axis_tready),
        .m_axis_tdata (m1_axis_tdata),  .m_axis_tlast (m1_axis_tlast)
    );

    axis_fifo #(.DEPTH(16), .DATA_W(DATA_W)) u_d16 (
        .clk(clk), .rst(rst),
        .s_axis_tvalid(s2_axis_tvalid), .s_axis_tready(s2_axis_tready),
        .s_axis_tdata (s2_axis_tdata),  .s_axis_tlast (s2_axis_tlast),
        .m_axis_tvalid(m2_axis_tvalid), .m_axis_tready(m2_axis_tready),
        .m_axis_tdata (m2_axis_tdata),  .m_axis_tlast (m2_axis_tlast)
    );

    axis_fifo #(.DEPTH(257), .DATA_W(DATA_W)) u_d257 (
        .clk(clk), .rst(rst),
        .s_axis_tvalid(s3_axis_tvalid), .s_axis_tready(s3_axis_tready),
        .s_axis_tdata (s3_axis_tdata),  .s_axis_tlast (s3_axis_tlast),
        .m_axis_tvalid(m3_axis_tvalid), .m_axis_tready(m3_axis_tready),
        .m_axis_tdata (m3_axis_tdata),  .m_axis_tlast (m3_axis_tlast)
    );
