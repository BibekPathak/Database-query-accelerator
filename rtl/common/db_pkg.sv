...
package db_pkg;
    ...
    localparam int COLUMN_ADDR_W =
        (NUM_ROWS <= 2) ? 1 : $clog2(NUM_ROWS);

    // Address width for GROUP_BY_BUCKETS hash buckets.
    localparam int GROUP_BY_ADDR_W =
        (GROUP_BY_BUCKETS <= 2) ? 1 : $clog2(GROUP_BY_BUCKETS);
    ...
    function automatic logic pred_apply(
        input pred_op_e                 op,
        input logic [COLUMN_WIDTH-1:0]  a,
    ...
    function automatic logic pred_combine(
        input logic     lhs,
        input logic_op_e op,
        input logic     rhs
    );
